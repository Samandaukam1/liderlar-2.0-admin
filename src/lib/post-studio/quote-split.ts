...
import type { LaidOutBlock, LaidOutLine, LaidOutRun } from "./types.ts";
...
/**
 * Rewrites a laid-out quote block's runs so the leading half carries
 * `accentFill`. Runs are positioned by measuring the prefix that precedes them
 * on their own line, which keeps every run's x exact under the same metrics the
 * wrap used — no re-measurement drift between preview and final render.
 */
export function applyQuoteColorSplit(
  ...
  const lines: LaidOutLine[] = block.lines.map((line) => {
    ...
    let runStart = 0;
    let runFill = wordCursor < splitAt ? accentFill : baseFill;
    ...
    const flush = (endExclusive: number) => {
      ...
      runs.push({ text, x: line.x + offset, fill: runFill });
      runStart = endExclusive;
    };

    for (let i = 0; i < lineWords.length; i += 1) {
      const fill = wordCursor + i < splitAt ? accentFill : baseFill;
      if (fill !== runFill) {
        flush(i);
        runFill = fill;
      }
